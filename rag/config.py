...
# 回复生成专用 key（与意图判断/Agent 编排的 key 分离，避免相互干扰；缺省回退主 key）
REPLY_API_KEY = os.getenv("REPLY_API_KEY", DEEPSEEK_API_KEY)

# ---- 模型名称 ----
EMBEDDING_MODEL = os.getenv("EMBEDDING_MODEL", "text-embedding-v4")
RERANKER_MODEL = os.getenv("RERANKER_MODEL", "qwen3-rerank")
LLM_MODEL = os.getenv("LLM_MODEL", "deepseek-chat")
# 意图识别使用的小模型（默认复用 LLM，可单独指定更廉价模型）
INTENT_MODEL = os.getenv("INTENT_MODEL", LLM_MODEL)
...
# ---- 大模型 Tool Calling ----
MAX_AGENT_ITERATIONS = int(os.getenv("MAX_AGENT_ITERATIONS", "3"))  # 工具调用最大轮次
